...
将 `breakMinutes: Int` 改为 `breakSeconds: Int`，内部统一用秒存储，UI 滑块下限扩展到 20 秒。
...
### 6. SettingsView.swift — 滑块 UI

替换原有的 `sliderRow` 为自定义的休息时长选择器：
